...
def get_download_urls(year, month, day=None, hour=None, minute=None):
    ...
    data_file_path = os.path.join(script_dir, '..', 'data', 'url_patterns.json')

    url_pattern_encrypted_data = json.load(open(data_file_path, "r"))
    ...
    for day in days_list:
        ...
        for pattern_codes in pattern_codes_list:
            ...
            base_url = f"https://archive.org/download/archiveteam-twitter-json-2011" if (
                        year_folder == '2011') else f"https://archive.org/download/archiveteam-twitter-stream-{year_folder}-{month_folder}"
