...
def process_tar_file(year, month, tar_file_url):
    ...
    save_to_folder = f"{ROOT_FOLDER_NAME}/{year}/{int(month):02d}"
    ...
    # filepaths = [f"{save_to_folder}/{tar_file_name}/{unquote(file_name)}.txt" for file_name in files]
    ...
    # cache completed downloading files in a txt file
    with open("completed.txt", 'a') as file:
        file.write(f"{tar_file_url}\n")

    print(f"[Finish everything] {tar_file_url}")


if __name__ == "__main__":
    tweetfiles = json.loads(open("tweettarfiles.json", "r").read())
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        for year in tweetfiles.keys():
            for month in tweetfiles[year].keys():
                ...
                for tar_file_url in tweetfiles[year][month]:
